...
static int client_login(int sock_listen) {
    ...
    for (client_id = 0; client_id < N_client; client_id++) {
        ...
        /* ユーザ情報を保存 */
        Client[client_id].sock = sock_accepted;  // 後で確認
...
static void send_question(char *question) {
    ...
    for (client_id = 0; client_id < N_client; client_id++) {
        Send(Client[client_id].sock, question, strlen(question),
             0);  // 第1引数を後で確認
    }
}
        ...
        for (client_id = 0; client_id < N_client; client_id++) {
            if (FD_ISSET(Client[client_id].sock, &readfds)) {
                ...
                if (check_answer(Buf)) { /* 解答が正しければ */
                    ...
                    Send(Client[client_id].sock, right_ans, strlen(right_ans),
                         0);
                    answered++;
                } else {
                    Send(Client[client_id].sock, wrong_ans, strlen(wrong_ans),
                         0);
                }
            }
        }
